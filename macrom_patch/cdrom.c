...
static const uint8_t bin2bcd[256] = {
...
static const uint8_t bcd2bin[256] = {
...
struct cdrom_drive_info {
    ...
    uint8_t toc[804];        /* TOC of currently inserted disk */
    uint8_t lead_out[3];    /* MSF address of lead-out track */
    uint8_t stop_at[3];    /* MSF address of audio play stopping point */

    uint8_t play_mode;    /* Audio play mode */
...
static void find_hfs_partition(cdrom_drive_info &info)
{
    ...
    for (int i=0; i<64; i++) {
        ...
        if (strcmp((char *)(map + 48), "Apple_HFS") == 0) {
            ...
            uint32_t num_blocks = (map[12] << 24) | (map[13] << 16) | (map[14] << 8) | map[15];
...
static bool position2msf(const cdrom_drive_info &info, uint16_t postype, uint32_t pos, bool stopping, uint8_t &m, uint8_t &s, uint8_t &f)
{
    switch (postype) {
        case 0:
            m = pos / (60 * 75);
        ...
        case 1:
            ...
            return true;
        case 2: {
            uint8_t track = bcd2bin[pos & 0xff];
            if (stopping)
                track++;
            for (int i=4; i<804; i+=8) {
...
int16_t CDROMOpen(uint32_t pb, uint32_t dce)
{
    D(bug("CDROMOpen\n"));
...
int16_t CDROMPrime(uint32_t pb, uint32_t dce)
{
...
int16_t CDROMControl(uint32_t pb, uint32_t dce)
{
    ...
    switch (code) {
        ...
        case 81:        /* Set poll freq */
            stw_p(dce + dCtlDelay, lduw_p(pb + csParam));
            return noErr;
    }
    ...
    switch (code) {
        ...
        case 22:        /* GetMediaIcon */
            stl_p(pb + csParam, CDROMIconAddr);
            return noErr;

        case 23:        /* drive_info */
            stl_p(pb + csParam, 0x00000b01);    /* Unspecified external removable SCSI disk */
            return noErr;

        case 70: {        /* SetPowerMode */
            uint8_t mode = ReadMacInt8(pb + csParam);
            if (mode > 3)
                return paramErr;
            else {
        ...
        case 76:        /* ModifyPostEvent */
            info->mount_non_hfs = lduw_p(pb + csParam);
            return noErr;

        case 79: {        /* Change block size */
            uint16_t size = lduw_p(pb + csParam);
        ...
        case 100: {        /* ReadTOC */
            ...
            switch (action) {
                ...
                case 3: {        /* Get track starting address */
                    uint32_t buf = ldl_p(pb + csParam + 2);
                    uint16_t buf_size = lduw_p(pb + csParam + 6);
                ...
                case 5:        /* Get session information */
                    stw_p(pb + csParam, 1);                            /* First session number */
                    stw_p(pb + csParam + 2, 1);                        /* Last session number */
                    stw_p(pb + csParam + 4, bin2bcd[info->toc[2]]);    /* First track number of last session */
        ...
        case 103: {        /* AudioTrackSearch */
            D(bug(" AudioTrackSearch postype %d, pos %08x, hold %d\n", lduw_p(pb + csParam), ldl_p(pb + csParam + 2), ReadMacInt16(pb + csParam + 6)));
            if (ReadMacInt8(info->status + dsDiskInPlace) == 0)
                return offLinErr;

            uint8_t start_m, start_s, start_f;
            if (!position2msf(*info, lduw_p(pb + csParam), ldl_p(pb + csParam + 2), false, start_m, start_s, start_f))
                return paramErr;
        ...
        case 104:        /* AudioPlay */
            ...
            if (lduw_p(pb + csParam + 6)) {
                /* Given stopping address */
                if (!position2msf(*info, lduw_p(pb + csParam), ldl_p(pb + csParam + 2), true, info->stop_at[0], info->stop_at[1], info->stop_at[2]))
                    return paramErr;
            } else {
        ...
        case 106:        /* AudioStop */
            D(bug(" AudioStop postype %d, pos %08lx\n", lduw_p(pb + csParam), ldl_p(pb + csParam + 2)));
            if (ReadMacInt8(info->status + dsDiskInPlace) == 0)
                return offLinErr;

            if (lduw_p(pb + csParam) == 0 && ldl_p(pb + csParam + 2) == 0) {
                /* Stop immediately */
        ...
        case 108: {        /* AudioScan */
            ...
            if (!SysCDScan(info->fh, start_m, start_s, start_f, lduw_p(pb + csParam + 6)))
                return paramErr;
            else
                return noErr;
        }
...
int16_t CDROMStatus(uint32_t pb, uint32_t dce)
{
    drive_vec::iterator info = get_drive_info(lduw_p(pb + ioVRefNum));
    uint16_t code = lduw_p(pb + csCode);
    D(bug("CDROMStatus %d\n", code));
        ...
        case 43: {    /* DriverGestalt */
            ...
            switch (sel) {
                case FOURCC('v','e','r','s'):    /* Version */
                    stl_p(pb + csParam + 4, 0x05208000);
                    break;
                case FOURCC('d','e','v','t'):    /* Device type */
                    stl_p(pb + csParam + 4, FOURCC('c','d','r','m'));
                    break;
                ...
                case FOURCC('b','o','o','t'):    /* Boot ID */
                    ...
                    stw_p(pb + csParam + 6, (uint16)CDROMRefNum);
                    break;
    ...
    switch (code) {
        case 6:            /* Return format list */
            if (lduw_p(pb + csParam) > 0) {
                ...
                stl_p(adr, SysGetFileSize(info->fh) / 512);    /* Number of blocks */
                stl_p(adr + 4, 0);                            /* heads/track/sectors */
                return noErr;
            } else
                return paramErr;
        ...
        case 70:        /* GetPowerMode */
            stw_p(pb + csParam, info->power_mode << 8);
            return noErr;

        case 95:        /* Get2KOffset */
            if (info->twok_offset > 0) {
                stw_p(pb + csParam, info->twok_offset);
                return noErr;
            } else
                return statusErr;

        case 96:        /* Get drive type */
            stw_p(pb + csParam, 3);            /* Apple CD 300 or newer */
            return noErr;

        case 98:        /* Get block size */
            stw_p(pb + csParam, info->block_size);
            return noErr;
        ...
        case 121:        /* Get CD features */
            stw_p(pb + csParam, 0x0200);    /* 300 KB/s */
            stw_p(pb + csParam, 0x0300);    /* SCSI-2, stereo */
            return noErr;
