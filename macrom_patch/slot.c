...
static void Offs(uint8_t type, uint32_t ptr)
{
    uint32_t offs = ptr - p;
    srom[p++] = type;
    srom[p++] = offs >> 16;
...
static void Rsrc(uint8_t type, uint32_t data)
{
    srom[p++] = type;
    srom[p++] = data >> 16;
    srom[p++] = data >> 8;
...
static void Long(uint32_t data)
{
    srom[p++] = data >> 24;
    srom[p++] = data >> 16;
    srom[p++] = data >> 8;
...
static void Word(uint16_t data)
{
    srom[p++] = data >> 8;
    srom[p++] = data;
}
...
#if 0
static uint32_t VModeParms(const monitor_desc &m, video_depth depth)
{
...
static uint32_t VModeDesc(uint32_t params, bool direct)
{
    uint32_t ret = p;
...
static uint32_t VMonitor(const monitor_desc &m, uint32_t videoType, uint32_t videoName, uint32_t vidDrvrDir, uint32_t gammaDir)
{
    uint32_t minorBase, minorLength;
    uint32_t vidModeParms1, vidModeParms2, vidModeParms4, vidModeParms8, vidModeParms16, vidModeParms32;
    uint32_t vidMode1, vidMode2, vidMode4, vidMode8, vidMode16, vidMode32;
    uint32_t ret;
...
int InstallSlotROM(uint8_t *rom, size_t rom_size)
{
    uint32_t boardType, boardName, vendorID, revLevel, partNum, date;
    uint32_t vendorInfo, sRsrcBoard;

    uint32_t videoType, videoName, videoDrvr, vidDrvrDir;
    uint32_t defaultGamma, gammaDir;

    uint32_t cpuType, cpuName, cpuMajor, cpuMinor, sRsrcCPU;

    uint32_t etherType, etherName, etherDrvr, etherDrvrDir, sRsrcEther;
    ...
    slot_rom_size = p;
    memcpy(rom + rom_size - slot_rom_size, srom, slot_rom_size);

    /* Calculate checksum */
    ChecksumSlotROM(rom, rom_size);
    return true;
}
...
void ChecksumSlotROM(uint8_t *rom, size_t rom_size)
{
    int i;
    /* Calculate CRC */
    uint8_t *p = rom + rom_size - slot_rom_size;
