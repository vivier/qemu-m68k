...
enum {    /* IOParam struct */
    ...
    ioPosMode = 44,
    ioPosOffset = 46,
    /* Wide positioning offset when ioPosMode has kWidePosOffsetBit set */
    ioWPosOffset = 46,
    SIZEOF_IOParam = 50
};
...
#if 0
/* Functions */
/* Enqueue QElem in list */
extern void EnqueueMac(uint32_t elem, uint32_t list);
/* Find first free drive number, starting at "num" */
extern int FindFreeDriveNumber(int num);
/* Mount volume with given file handle (see sys.h) */
extern void MountVolume(void *fh);
/* Calculate disk image file layout given file size and first 256 data bytes */
extern void FileDiskLayout(loff_t size, uint8_t *data,
                           loff_t *start_byte, loff_t *real_size);
/* DebugUtil() Replacement */
extern uint32_t DebugUtil(uint32_t Selector);
/* Convert time_t value to MacOS time */
extern uint32_t TimeToMacTime(time_t t);

/* Construct four-character-code */
#define FOURCC(a, b, c, d) (((uint32)(a) << 24) | ((uint32)(b) << 16) | \
                            ((uint32)(c) << 8) | (uint32)(d))
...
{
    /* Mac warm start flag */
    return ldl_phys(0xcfc) == FOURCC('W', 'L', 'S', 'C');
}
#endif /* 0 */
#endif
