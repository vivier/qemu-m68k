...
static target_fd_trans_t target_netlink_route_trans = {
    .target_to_host = netlink_route_target_to_host,
    .host_to_target = netlink_route_host_to_target,
};
...
static target_fd_trans_t target_netlink_audit_trans = {
    .target_to_host = netlink_audit_target_to_host,
    .host_to_target = netlink_audit_host_to_target,
};
...
static abi_long do_socket(int domain, int type, int protocol)
{
    ...
    if (ret >= 0) {
        ...
        if (domain == PF_NETLINK) {
            switch (protocol) {
            case NETLINK_ROUTE:
                target_fd_trans[ret] = &target_netlink_route_trans;
                break;
            case NETLINK_KOBJECT_UEVENT:
                /* nothing to do: messages are strings */
                break;
            case NETLINK_AUDIT:
                target_fd_trans[ret] = &target_netlink_audit_trans;
                break;
            default:
                close(ret);
                ret = -EPFNOSUPPORT;
                                      ...
                                      int flags, int send)
{
    ...
    if (send) {
        if (fd < target_fd_max &&
            target_fd_trans[fd] &&
            target_fd_trans[fd]->target_to_host) {
                ret = target_fd_trans[fd]->target_to_host(msg.msg_iov->iov_base,
                                                          msg.msg_iov->iov_len);
        } else {
        ...
        if (!is_error(ret)) {
            ...
            if (fd < target_fd_max &&
                target_fd_trans[fd] &&
                target_fd_trans[fd]->host_to_target) {
                ret = target_fd_trans[fd]->host_to_target(msg.msg_iov->iov_base,
                                                          msg.msg_iov->iov_len);
            } else {
                          ...
                          abi_ulong target_addr, socklen_t addrlen)
{
    ...
    if (fd < target_fd_max &&
        target_fd_trans[fd] &&
        target_fd_trans[fd]->target_to_host) {
            ret = target_fd_trans[fd]->target_to_host(host_msg, len);
            if (ret < 0) {
