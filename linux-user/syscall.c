...
static TargetFdFunc fd_trans_target_to_host(int fd)
{
    if (fd < target_fd_max && target_fd_trans[fd]) {
        return target_fd_trans[fd]->target_to_host;
    }
    return NULL;
}
                                      ...
                                      int flags, int send)
{
    ...
    if (send) {
        if (fd_trans_target_to_host(fd)) {
                ret = fd_trans_target_to_host(fd)(msg.msg_iov->iov_base,
                                                  msg.msg_iov->iov_len);
        } else {
        ...
        if (!is_error(ret)) {
            len = ret;
            if (fd_trans_host_to_target(fd)) {
                ret = fd_trans_host_to_target(fd)(msg.msg_iov->iov_base,
                                                 msg.msg_iov->iov_len);
            } else {
                          ...
                          abi_ulong target_addr, socklen_t addrlen)
{
    ...
    if (!host_msg)
        return -TARGET_EFAULT;
    if (fd_trans_target_to_host(fd)) {
            ret = fd_trans_target_to_host(fd)(host_msg, len);
            if (ret < 0) {
