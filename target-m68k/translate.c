...
QREG_##name##H = tcg_global_mem_new_i32(TCG_AREG0, offsetof(CPUM68KState, offset##h), #name); \
QREG_##name##L = tcg_global_mem_new_i64(TCG_AREG0, offsetof(CPUM68KState, offset##l), #name); \
} while (0);
#include "qregs.def"
#undef DEFO32
#undef DEFO64
...
static void m68k_link(DisasContext *s, uint16_t insn, int32_t offset)
{
    TCGv reg;
    TCGv tmp;
...
static void addx(TCGv res, TCGv src, TCGv reg)
{
    TCGv z, tmp1, tmp2;
    ...
    z = tcg_const_i32(0);
    tcg_gen_add2_i32(res, QREG_CC_X, src, z, QREG_CC_X, z);
    tcg_gen_add2_i32(res, QREG_CC_X, reg, z, res, QREG_CC_X);
    tcg_temp_free(z);
...
DISAS_INSN(shift_im)
{
    ...
    if (shift_left) {
        tcg_gen_shri_i32(QREG_CC_X, reg, 8 * opsize_bytes(opsize) - count);
        tcg_gen_shli_i32(reg, reg, count);
    } else {
        tcg_gen_shri_i32(QREG_CC_X, reg, count - 1);
        if (arith) {
            tcg_gen_sari_i32(reg, reg, count);
        } else {
            tcg_gen_shri_i32(reg, reg, count);
        }
    }
    tcg_gen_andi_i32(QREG_CC_X, QREG_CC_X, 1);
    /* compute flags */
    tcg_gen_mov_i32(QREG_CC_DEST, QREG_CC_X); /* C always equal to X */

    tmp = tcg_temp_new_i32();
    tcg_gen_setcondi_i32(TCG_COND_LT, tmp, reg, 0); /* N */
    tcg_gen_deposit_i32(QREG_CC_DEST, QREG_CC_DEST, tmp, 3, 1);
    ...
    if (arith && m68k_feature(s->env, M68K_FEATURE_M68000)) {
        tcg_gen_xor_i32(old_sign, old_sign, tmp); /* V */
    } else {
        tcg_gen_movi_i32(old_sign, 0); /* V */
    }
    tcg_gen_deposit_i32(QREG_CC_DEST, QREG_CC_DEST, old_sign, 1, 1);
    tcg_temp_free(old_sign);

    tcg_gen_setcondi_i32(TCG_COND_EQ, tmp, reg, 0); /* Z */
    tcg_gen_deposit_i32(QREG_CC_DEST, QREG_CC_DEST, tmp, 2, 1);
...
DISAS_INSN(shift_mem)
{
    ...
    old_sign = tcg_temp_new_i32();
    tcg_gen_setcondi_i32(TCG_COND_LT, old_sign, src, 0); /* N */
    if (shift_left) {
        tcg_gen_shri_i32(QREG_CC_X, src, 15);
        tcg_gen_andi_i32(QREG_CC_X, QREG_CC_X, 1);
        tcg_gen_shli_i32(dest, src, 1);
    } else {
        tcg_gen_andi_i32(QREG_CC_X, src, 1);
        if (arith) {
            tcg_gen_sari_i32(dest, src, 1);
        } else {
    ...
    tmp = tcg_temp_new_i32();
    tcg_gen_setcondi_i32(TCG_COND_LT, tmp, dest, 0); /* N */
    tcg_gen_deposit_i32(QREG_CC_DEST, QREG_CC_DEST, tmp, 3, 1);
    /* Note that ColdFire always clears V, while M68000 sets it for
       a change in the sign bit.  */
    if (arith && m68k_feature(s->env, M68K_FEATURE_M68000)) {
    ...
    tcg_gen_setcondi_i32(TCG_COND_EQ, tmp, dest, 0); /* Z */
    tcg_gen_deposit_i32(QREG_CC_DEST, QREG_CC_DEST, tmp, 2, 1);
