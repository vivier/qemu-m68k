...
static TCGv gen_ea(CPUM68KState *env, DisasContext *s, uint16_t insn,
                   int opsize, TCGv val, TCGv *addrp,
                   TCGv *post, ea_what what, int index)
{
    TCGv reg;
    TCGv result;
    uint32_t offset;
    ...
    case 5: /* Indirect displacement.  */
    case 6: /* Indirect index + displacement.  */
        return gen_ea_once(env, s, insn, opsize, val, addrp, what, index);
    case 7: /* Other */
        switch (insn & 7) {
        case 0: /* Absolute short.  */
        case 1: /* Absolute long.  */
...
static void gen_jmpcc(DisasContext *s, int cond, TCGLabel *l1, TCGv flags)
{
    TCGv tmp;
...
#define SRC_EA(env, result, opsize, op_sign, addrp) do { \
    result = gen_ea(env, s, insn, opsize, NULL_QREG, addrp, \
                    NULL, op_sign ? EA_LOADS : EA_LOADU, IS_USER(s)); \
...
DISAS_INSN(bitop_reg)
{
    ...
    cc_nz = tcg_temp_new();
    tcg_gen_shr_i32(cc_nz, src1, src2);
    tcg_gen_andi_i32(cc_nz, cc_nz, 1);
    tcg_gen_shli_i32(cc_nz, cc_nz, 2);
    ...
    tcg_gen_xor_i32(QREG_CC_DEST, QREG_CC_DEST, cc_nz);
    set_cc_op(s, CC_OP_FLAGS);
}
...
DISAS_INSN(bitop_im)
{
    ...
    cc_nz = tcg_temp_new();
    if (bitnum > 2)
        tcg_gen_shri_i32(cc_nz, src1, bitnum - 2);
    else if (bitnum < 2)
        tcg_gen_shli_i32(cc_nz, src1, 2 - bitnum);
    else
        tcg_gen_mov_i32(cc_nz, src1);
    tcg_gen_andi_i32(cc_nz, cc_nz, CCF_Z);
    /* bitop */
    tmp = tcg_temp_new();
    if (op) {
        switch (op) {
...
DISAS_INSN(arith_im)
{
    ...
    switch (opsize) {
    case OS_BYTE:
        im = tcg_const_i32(read_im8(env, s));
        break;
    case OS_WORD:
        im = tcg_const_i32(read_im16(env, s));
        break;
    case OS_LONG:
...
DISAS_INSN(bitfield_mem)
{
    ...
    gen_helper_bitfield_load(bitfield, cpu_env, tmp, offset, width);
    ...
    case 2: /* bfchg */
        ...
        gen_helper_bitfield_store(cpu_env, tmp, offset, width, bitfield);
        break;
    case 3: /* bfexts */
        shift = tcg_temp_new_i32();
        tcg_gen_sub_i32(shift, tcg_const_i32(32), width);
...
DISAS_INSN(fpu)
{
    ...
    case 5: /* fmove from control register.  */
        ...
        if (MODE(insn) == 0) {
            ...
            if (ctrl & 1) { /* FPIAR */
                tcg_gen_movi_i32(DREG(insn, 0), 0);
            }
            return;
        }
        if (MODE(insn) == 1) {
